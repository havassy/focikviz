...
function showQuestion(question) {
    ...
    if (question.image) {
        ...
        img.src = `kepek/${currentTopic}/${question.image}`;
        img.className = 'question-image';
        img.alt = 'Kérdéshez tartozó kép';
...
function generateCertificate(name, sessionKey) {
    ...
    const topicEmojis = {
        ...
        'kozetbolygo': {
            all: ['🪨', '⛰️', '🌋', '🏔️', '💎', '🗻', '⚒️', '🔨', '🏜️', '⛏️', '🏞️', '🪵', '🌍', '🌎', '🌏'],
            yellow: ['🏜️'], // Sivatag (sárga)
            earth: ['🌍', '🌎', '🌏']
        },
        'legkor': {
            all: ['☁️', '🌪️', '⛈️', '🌦️', '🌬️', '💨', '🌤️', '⛅', '🌥️', '🌨️', '🌩️', '⚡', '🌧️', '❄️', '💧'],
            yellow: ['🌤️'], // Napos (sárga)
            earth: []
        },
        'vizburok': {
            all: ['💧', '🌊', '🏖️', '🐠', '🌀', '⛵', '🚢', '🏄‍♂️', '🐋', '🐟', '💦', '🌐', '🏝️', '⛴️', '🦈'],
            yellow: ['🏖️'], // Strand (sárga homok)
            earth: []
        },
        'geoszferak': {
            all: ['🌍', '🌎', '🌏', '🗺️', '🧭', '🌐', '🗾', '🏞️', '🌄', '🏕️', '⛰️', '🗻'],
    ...
    // Alapértelmezett emoji lista (ha téma nem található)
    const defaultEmojis = {
        all: ['🌟', '⭐', '✨', '🌍', '🌎', '🌏', '☄️', '🚀', '🛸', '🌌', '🔭'],
        yellow: ['🌟', '⭐', '✨'],
        earth: ['🌎', '🌍', '🌏']
    ...
    // Emoji lista lekérése az aktuális témakör alapján
    const currentEmojis = topicEmojis[currentTopic] || defaultEmojis;
    const allEmojis = currentEmojis.all;
    const yellowEmojis = currentEmojis.yellow;
    const earthEmojis = currentEmojis.earth;
    ...
    function getRandomEmojis(count, isGold = false) {
        ...
        let availableEmojis = isGold 
            ? allEmojis.filter(e => !yellowEmojis.includes(e))
            : allEmojis;
...
function updateTotalPointsDisplay() {
    ...
    updateMissingPointsDisplay();
}

function updateMissingPointsDisplay() {
    const minPoints = window.minPoints || 30;
    const missing = Math.max(0, minPoints - earnedPoints);
    document.querySelectorAll('#missingPoints').forEach(el => {
